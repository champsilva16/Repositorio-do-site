...
if(isset($_POST['submit'])){
  ...
  $query="select id_tra,trabalho,email_tra,titulo,descricao,dta from a14949_trabalhos where email_tra=? and standby=1";
  ...
  $my = new mysqli($bd_host,$bd_user,$bd_password,$bd_database);
    ...
    while($sta->fetch()){
      ...
      $trab2='"'.$trab.'"';
        ...
        echo "<input type='hidden' name='foto'/>";
        echo '<tbody role="rowgroup">
        <tr role="row">
        <td role="cell"><input type="text" class="form-control"name="id" value="'.$nome3.'" readonly></td>
      ...
      echo '<td role="cell"><input type="submit"  name="eliminar1" value="eliminar " class="btn btn-danger"  ></td>';
 ...
 echo' </tbody>
    ...
    $query="select id_tra,trabalho,email_tra,titulo,descricao,dta from a14949_trabalhos where standby=1";
    $sta=$liga->prepare($query);
    $sta->execute();
    $sta->bind_result($id,$trab,$email,$tit,$desc,$dta);
